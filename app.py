...
engine = create_engine('postgresql://ec4_user:DS4A@extendedcase-4.cmb1am1ofodt.us-east-2.rds.amazonaws.com/extendedcase4_db')
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.DatePickerRange(
                                        ...
                                        start_date=dt(df['Entry time'].min().year, df['Entry time'].min().month, df['Entry time'].min().day),
										end_date=dt(df['Entry time'].max().year, df['Entry time'].max().month, df['Entry time'].max().day)
...
def filter_df(df,Exchange,Margin,start_date,end_date):
    ...
    return df[(df['Exchange']==Exchange) & (df['Margin']==Margin) & (df['Entry time']<end_date) & (df['Entry time']>start_date)]
...
def update_btc_daily(exchange, leverage, start_date, end_date):
	dff=filter_df(df,exchange,leverage,start_date,end_date)
...
def update_balance(exchange, leverage, start_date, end_date):
	...
	return {'data' :[go.Scatter(
			...
			'layout':  go.Layout(title=f'Portfolio over time',
                       height=400,
                       legend={'x': 0, 'y': 1}),
